...
public class Archer : WarriorEntity
{
    [SerializeField]
    private GameObject arealDamagePrefab;
    [SerializeField]
    private GameObject ArrowPrefab;
    new public void Attack(Entity victim)
    {
        Debug.Log("I TRY TO ATTACK");

        Vector3 pos = victim.transform.position;
        GameObject arealDmg = Instantiate(arealDamagePrefab, pos, Quaternion.identity);
        ArealDamage dmgScript = arealDmg.GetComponent<ArealDamage>();
        dmgScript.Damage = this.Damage;
        dmgScript.DamageDelay = 20;

        GameObject arrow = Instantiate(ArrowPrefab, gameObject.transform.position, Quaternion.identity);
        Arrow arrowScript = arrow.GetComponent<Arrow>();
        arrowScript.EndLocation = new Vector2(victim.transform.position.x, victim.transform.position.y);
        arrowScript.FlightDuration = 20;
    }
